...
export class ${convertToClassName(table)} extends Entity<${convertToQueryProxyName(table)}> {
	...
	${body.trim()}
		...
		sets.push(`static ${convertToModelName(table)}: DbSet<${convertToClassName(table)}, ${convertToQueryProxyName(table)}> = new DbSet<${convertToClassName(table)}, ${convertToQueryProxyName(table)}>();`);
