...
export class QueryJoin<TModel extends Entity<TQueryModel> | View<TQueryModel>, TQueryModel extends QueryProxy> {
	...
	toSQL() {
		return `LEFT JOIN ${this.table} AS ${this.extent.name} ON${this.query.set instanceof Entity && this.query.set.$$meta.active ? ` ${this.extent.name}.${this.query.set.$$meta.active} AND` : ""} ${this.extent.name}.${this.column} = ${this.from.name}.id`;
	}
}
